...
function checkIfExist($array, $datenbankVerbindung){
    ...
    // mysqli_affected_rows zählt, wie viele Zeilen gefunden wurden
      ...
      $sql2 = "SELECT * FROM itc_kunden WHERE vorname='$array[vorname]' and nachname='$array[name]' and gbdatum='$array[gbdatum]'";
      $result2 = mysqli_query($datenbankVerbindung, $sql2);
      $affected2 = mysqli_affected_rows($datenbankVerbindung);

      if($affected2 == 0){
        $datenbankVerbindung -> query("INSERT INTO itc_kunden(vorname, nachname, geschlecht, gbdatum, art, adresse) VALUES ('$array[vorname]', '$array[name]', '$array[geschlecht]', '$array[gbdatum]', '$array[privatGesch]', '$adressId')"); 
